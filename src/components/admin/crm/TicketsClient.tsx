    ...
    <div className="mx-auto w-full max-w-6xl px-5 py-12 sm:px-8">
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <h1 className="font-display text-3xl font-semibold tracking-tight text-white">Tickets</h1>
        ...
        <p className="text-sm text-white/60">Track client support requests and internal issues.</p>
      </div>
      ...
      {showNew && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-[3px] bg-[#111827] p-6 shadow-xl">
            <h2 className="font-display text-xl font-semibold text-white">New Ticket</h2>
            <form onSubmit={handleNewTicket} className="mt-4 flex flex-col gap-4">
              <div>
                ...
                <input type="text" required value={newTicket.subject} onChange={(e) => setNewTicket({ ...newTicket, subject: e.target.value })} className="w-full rounded-[3px] border border-[#1E293B] bg-[#111827] px-3 py-2 text-sm text-white focus:outline-none focus:border-brand" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-white/60 mb-1">Description *</label>
                <textarea required value={newTicket.description} onChange={(e) => setNewTicket({ ...newTicket, description: e.target.value })} rows={4} className="w-full rounded-[3px] border border-[#1E293B] bg-[#111827] px-3 py-2 text-sm text-white focus:outline-none focus:border-brand" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-white/60 mb-1">Company *</label>
                <select required value={newTicket.companyId} onChange={(e) => setNewTicket({ ...newTicket, companyId: e.target.value })} className="w-full rounded-[3px] border border-[#1E293B] bg-[#111827] px-3 py-2 text-sm text-white focus:outline-none focus:border-brand">
                  <option value="">Select company</option>
                  {companies.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold text-white/60 mb-1">Contact (optional)</label>
                <select value={newTicket.contactId} onChange={(e) => setNewTicket({ ...newTicket, contactId: e.target.value })} className="w-full rounded-[3px] border border-[#1E293B] bg-[#111827] px-3 py-2 text-sm text-white focus:outline-none focus:border-brand">
                  <option value="">None</option>
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold text-white/60 mb-1">Project (optional)</label>
                <select value={newTicket.projectId} onChange={(e) => setNewTicket({ ...newTicket, projectId: e.target.value })} className="w-full rounded-[3px] border border-[#1E293B] bg-[#111827] px-3 py-2 text-sm text-white focus:outline-none focus:border-brand">
                  <option value="">None</option>
              ...
              <div>
                ...
                <select value={newTicket.priority} onChange={(e) => setNewTicket({ ...newTicket, priority: e.target.value })} className="w-full rounded-[3px] border border-[#1E293B] bg-[#111827] px-3 py-2 text-sm text-white focus:outline-none focus:border-brand">
        ...
        {tickets.length === 0 ? (
          ...
          <ul className="divide-y divide-[#1E293B]">
            {tickets.map((ticket: any) => (
              <li key={ticket.id} className="flex items-center justify-between gap-4 p-4 hover:bg-white/5">
                ...
                {ticket.assignee && <span className="text-xs text-white/50">{ticket.assignee.name}</span>}
              </li>
            ))}
          </ul>
        )}
      ...
      {totalPages > 1 && (
        <div className="mt-6 flex items-center justify-between">
          <span className="text-sm text-white/60">Page {page} of {totalPages} — {total} tickets</span>
          <div className="flex gap-2">
            <button onClick={() => { setPage(p => Math.max(1, p - 1)); fetchTickets(); }} disabled={page === 1 || loading} className="rounded-[3px] border border-[#1E293B] px-3 py-1.5 text-sm text-white disabled:opacity-50">Prev</button>
            <button onClick={() => { setPage(p => Math.min(totalPages, p + 1)); fetchTickets(); }} disabled={page === totalPages || loading} className="rounded-[3px] border border-[#1E293B] px-3 py-1.5 text-sm text-white disabled:opacity-50">Next</button>
          </div>
        </div>
      )}
    </div>
