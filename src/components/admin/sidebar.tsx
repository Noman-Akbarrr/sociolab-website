...
export default function Sidebar({
  ...
  const visibleKeys = getVisibleNavItems(role);
  ...
  return (
    <aside
      className="fixed inset-y-0 left-0 z-40 flex flex-col bg-[#111827] text-white transition-[width] duration-200 ease-in-out"
      style={{ width: collapsed ? "4rem" : "16rem" }}
    >
      {/* Logo */}
      <div className="flex h-14 items-center border-b border-white/10 px-4">
        <Link href="/admin" className="flex items-center gap-3 overflow-hidden">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-brand font-display text-sm font-bold text-white">
            S
          </span>
          {!collapsed && (
