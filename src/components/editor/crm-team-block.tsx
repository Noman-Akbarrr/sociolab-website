    ...
    return (
      <Section className="bg-mist">
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="flex h-full flex-col overflow-hidden rounded-[3px] border border-[#1E293B] bg-[#111827] animate-pulse">
              <div className="relative aspect-[4/5] w-full bg-[#0F172A]" />
              <div className="flex flex-col gap-1 p-6">
                <div className="h-6 w-1/3 bg-[#1E293B] rounded" />
      ...
      <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {members.length === 0 ? (
          ...
          members.map((member: TeamMember, i: number) => (
            <div key={member.id} className="flex h-full flex-col overflow-hidden rounded-[3px] border border-[#1E293B] bg-[#111827]">
              <div className="relative aspect-[4/5] w-full bg-[#0F172A]">
                {member.photo ? (
              ...
              <div className="flex flex-col gap-1 p-6">
                ...
                {(member.linkedin || member.twitter) && (
                  <div className="mt-3 flex items-center gap-3">
                    {member.linkedin && (
                      <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="text-white/50 hover:text-[#FF5500]">
                    ...
                    {member.twitter && (
                      <a href={member.twitter} target="_blank" rel="noopener noreferrer" className="text-white/50 hover:text-[#FF5500]">
