    ...
    return (
      <Section className="bg-mist">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2">
          {[1, 2].map((i) => (
            <figure key={i} className="flex h-full flex-col justify-between gap-8 rounded-[3px] bg-[#111827] p-8 animate-pulse">
              <div className="h-16 w-full bg-[#1E293B] rounded" />
              <div className="flex items-center gap-3">
                <div className="grid size-11 place-items-center rounded-full bg-[#FF5500]" />
      ...
      <div className="mt-12 grid gap-5 md:grid-cols-2">
        {testimonials.length === 0 ? (
          ...
          testimonials.map((t: Testimonial, i: number) => (
            <figure key={t.id} className="flex h-full flex-col justify-between gap-8 rounded-[3px] bg-[#111827] p-8">
              ...
              <figcaption className="flex items-center gap-3">
                ...
                <span>
                  ...
                  <span className="block text-xs text-white/55">{t.role}{t.company ? `, ${t.company}` : ""}</span>
                </span>
              </figcaption>
            </figure>
          ))
