  ...
  return (
    <div className="px-8 py-10">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <Link href="/admin/deals" className="text-sm text-white/50 hover:underline">&larr; Back to Deals</Link>
          <h1 className="mt-1 font-display text-2xl font-semibold tracking-tight text-white">{deal.title}</h1>
        </div>
        <select
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="rounded-[3px] border border-[#1E293B] bg-[#111827] p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <Link href={`/admin/clients/${deal.company.id}`} className="font-display text-lg font-semibold text-white hover:text-brand">
                  {deal.company.name}
                </Link>
              ...
              {deal.activities.length === 0 ? (
                ...
                deal.activities.map((activity: any) => (
                  <div key={activity.id} className="p-5">
                    ...
                    {activity.body && <p className="text-sm text-white/60 mt-2">{activity.body}</p>}
                  </div>
                ))
              )}
            </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="rounded-[3px] border border-[#1E293B] bg-[#111827] p-5">
            ...
            <dl className="space-y-3 text-sm">
              <div><dt className="text-white/50">Value</dt><dd className="font-display text-xl font-semibold text-brand">{formatCurrency(deal.value)}</dd></div>
              <div><dt className="text-white/50">Probability</dt><dd className="font-semibold">{deal.probability}%</dd></div>
              <div><dt className="text-white/50">Weighted</dt><dd className="font-display text-xl font-semibold text-brand">{formatCurrency(Math.round(deal.value * deal.probability / 100))}</dd></div>
              {deal.expectedClose && <div><dt className="text-white/50">Expected Close</dt><dd className="font-semibold">{new Date(deal.expectedClose).toLocaleDateString()}</dd></div>}
            </dl>
          </div>

          <div className="rounded-[3px] border border-[#1E293B] bg-[#111827] p-5">
            <h3 className="font-display text-sm font-semibold text-white mb-4">Actions</h3>
            <div className="flex flex-col gap-2">
              ...
              {deal.projects.length > 0 && (
                <Link href={`/admin/clients/${deal.company.id}`} className="rounded-[3px] border border-[#1E293B] px-3 py-2 text-sm font-medium text-white text-center hover:border-brand">
                  View Project
                </Link>
              )}
            </div>
          ...
          <div className="rounded-[3px] border border-[#1E293B] bg-[#111827] p-5">
            ...
            <form onSubmit={async (e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); await fetch("/admin/api/crm/activities", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "note", subject: "Note added", body: fd.get("body"), dealId: deal.id }) }); window.location.reload(); }} className="flex flex-col gap-2">
              <textarea name="body" rows={3} required className="rounded-[3px] border border-[#1E293B] bg-[#111827] px-3 py-2 text-sm text-white focus:outline-none focus:border-brand resize-none" placeholder="Add a note..." />
